from flask import Flask, render_template, request, jsonify
import time
import atexit

app = Flask(__name__)

print("\n" + "="*60)
print("🤖 ROBOT CONTROL - Pi 5 LED FIX")
print("="*60)

# Import motor
from motor import Ordinary_Car

# Import LED with Pi 5 fix
try:
    from led_pi5_fix import Led
    print("✅ LED module loaded")
except ImportError as e:
    print(f"❌ LED import error: {e}")
    print("   Make sure led_pi5_fix.py exists")

PWM = Ordinary_Car()
print("✅ Motor initialized")

LED = Led()
print("✅ LED initialized (Pi 5 compatible)")

print("="*60 + "\n")

def robot_forward():
    PWM.set_motor_model(1, 0, 1, 0)
    print("  🔴 FORWARD")

def robot_backward():
    PWM.set_motor_model(0, 1, 0, 1)
    print("  🔴 BACKWARD")

def robot_left():
    PWM.set_motor_model(0, 1, 1, 0)
    print("  🔴 LEFT")

def robot_right():
    PWM.set_motor_model(1, 0, 0, 1)
    print("  🔴 RIGHT")

def robot_stop():
    PWM.set_motor_model(0, 0, 0, 0)
    print("  ⚫ STOP")

def led_red():
    try:
        LED.ledIndex(0xFF, 255, 0, 0)
        print("  🔴 LED RED")
    except Exception as e:
        print(f"  ⚠️ LED red failed: {e}")

def led_green():
    try:
        LED.ledIndex(0xFF, 0, 255, 0)
        print("  🟢 LED GREEN")
    except Exception as e:
        print(f"  ⚠️ LED green failed: {e}")

def led_blue():
    try:
        LED.ledIndex(0xFF, 0, 0, 255)
        print("  🔵 LED BLUE")
    except Exception as e:
        print(f"  ⚠️ LED blue failed: {e}")

def led_off():
    try:
        LED.ledIndex(0xFF, 0, 0, 0)
        print("  ⚫ LED OFF")
    except Exception as e:
        print(f"  ⚠️ LED off failed: {e}")

@app.route("/")
def index():
    return render_template("index.html")

@app.route("/status")
def status():
    return jsonify({'status': 'ok', 'robot': 'connected'})

@app.route("/move/<int:robot_id>/<direction>")
def move_robot(robot_id, direction):
    print(f"\n🤖 Robot: {direction.upper()}")
    
    if direction != 'stop':
        led_green()
    
    if direction == 'forward':
        robot_forward()
    elif direction == 'backward':
        robot_backward()
    elif direction == 'left':
        robot_left()
    elif direction == 'right':
        robot_right()
    elif direction == 'stop':
        robot_stop()
        led_off()
    
    return jsonify({'status': 'ok', 'direction': direction})

@app.route("/move/all/<direction>")
def move_all_robots(direction):
    return move_robot(0, direction)

@app.route("/led/red")
def set_red():
    led_red()
    return jsonify({'status': 'ok'})

@app.route("/led/green")
def set_green():
    led_green()
    return jsonify({'status': 'ok'})

@app.route("/led/blue")
def set_blue():
    led_blue()
    return jsonify({'status': 'ok'})

@app.route("/led/off")
def set_off():
    led_off()
    return jsonify({'status': 'ok'})

@atexit.register
def cleanup():
    print("\n🧹 Cleaning up...")
    robot_stop()
    led_off()
    try:
        PWM.close()
    except:
        pass

if __name__ == "__main__":
    print("🚀 Server: http://10.243.53.235:5005")
    print("📡 Test LED: http://10.243.53.235:5005/led/red")
    print("="*60 + "\n")
    app.run(host="0.0.0.0", debug=False, port=5005)
